...
//                        true, true);
        ...
        var fillArcTypesAndDisplayForm = function (evt, originType, targetType, arcType, arcId) {
            ...
            //Yan: don't open the arc (i.e., Flow) dialog, just annotate directly to save time
            //console.log(arcForm.dialog('option', 'title'));
            if(arcForm.dialog('option', 'title') =="Edit Annotation" ) {
        ...
        var addSpanTypesToDivInner = function ($parent, types, category) {
            ...
            $.each(types, function (typeNo, type) {
                if (type === null) {
                    ...
                    spanBgColor = Util.adjustColorLightness(spanBgColor, spanBoxTextBgColorLighten);
                    if (name.indexOf("TP (") != -1) {
                        var $label = $('<label class="span_type_label ui-corner-all"/>')
                            .attr('for', 'span_' + type.type).text(name.substring(name.indexOf("(") + 1, name.indexOf(")")));
                    }else
                        var $label = $('<label class="span_type_label ui-corner-all"/>')
                            .attr('for', 'span_' + type.type).text(name)
                    if (type.unused) {
                        $input.attr({
                            disabled: 'disabled',
                            unused: 'unused'
        ...
        var addAttributeTypesToDiv = function ($top, types, category) {
            $.each(types, function (attrNo, attr) {
                ...
                if (attr.unused) {
                    ...
                    var $div = $('<div />');
                    var $select = $('<select size="5" style="overflow:auto; margin-left: 30px; width: 150px; height: 100px;" id="' + attrId + '"  category="' + category + '"/>');
        ...
        var onMultiAttrChange = function (evt) {
            if ($(this).val() == '') {
                ...
                if (evt.target.selectedIndex) {
                    ...
                    //attempt at simulating a click
                    if (spanForm.dialog('isOpen')){
                        $('#span_form-ok').trigger('click');
                    }
                } else {
